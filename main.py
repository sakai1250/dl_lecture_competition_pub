...
def run(args: DictConfig):
    ...
    for epoch in range(args.epochs):
        ...
        torch.save(model.state_dict(), os.path.join(logdir, "model_last_main.pt"))
        if args.use_wandb:
            wandb.log({"train_loss": np.mean(train_loss), "train_acc": np.mean(train_acc), "val_loss": np.mean(val_loss), "val_acc": np.mean(val_acc)})
        
        if np.mean(val_acc) > max_val_acc:
            cprint("New best.", "cyan")
            torch.save(model.state_dict(), os.path.join(logdir, "model_best_main.pt"))
    ...
    # ----------------------------------
    model.load_state_dict(torch.load(os.path.join(logdir, "model_best_main.pt"), map_location=args.device))
